...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    for (int i = 0; i < iteration; i++) {
        int total_update = 0;
        memset(y, 0, sizeof(y));
        memset(sum_epsilon, 0, sizeof(sum_epsilon[0][0]) * MAX_STATE * MAX_STATE);
        memset(sum_condition_gamma, 0,
               sizeof(sum_condition_gamma[0][0]) * MAX_STATE * MAX_OBSERV);
        ...
        while (fscanf(sequence_file, "%s", y) == 1) {
            int T = strlen(y);
            memset(gamma, 0, sizeof(gamma[0][0]) * MAX_STATE * MAX_SEQ);
            memset(alpha, 0, sizeof(alpha[0][0]) * MAX_SEQ * MAX_STATE);
            memset(beta, 0, sizeof(beta[0][0]) * MAX_STATE * MAX_SEQ);
            memset(epsilon, 0, sizeof(epsilon[0][0][0]) * MAX_STATE * MAX_STATE * MAX_SEQ);
            for (int i = 0; i < hmm.state_num; i++) {
                //observation[MAX_OBSERV][MAX_STATE];
                //Forward
            ...
            for (int t = 0; t < T; t++) {
                for (int i = 0; i < hmm.state_num; i++) {
                    ...
                    alpha[i][t + 1] = sum_alpha * hmm.observation[y[t] - 'A'][i];
                }
            }
            //backward
            /*{ \beta _{i}(T)=1,}
        ...
        for (int i = 0; i < hmm.state_num; i++) {
            ...
            // printf("sum_gamma_init[%d]=%lf \n", i, sum_gamma_init[i]);
        }
        for (int i = 0; i < hmm.state_num; i++) {
            for (int v_k = 0; v_k < hmm.observ_num; v_k++) {
                hmm.observation[v_k][i] = sum_condition_gamma[v_k][i] / sum_gamma[i];
                // printf("sum_condition_gamma[%d][%d]=%lf \n", i, v_k, sum_condition_gamma[v_k][i]);
            }
        }
        ...
        for (int i = 0; i < hmm.state_num; i++) {
            for (int j = 0; j < hmm.state_num; j++) {
                ...
                // printf("sum_epsilon[%d][%d]=%lf \n", i, j, sum_epsilon[i][j]);
            }
        }
        // for (int i = 0; i < hmm.state_num; i++) {
        //     memcpy(hmm.transition[i], a_new[i], sizeof(a_new[i]));
        // }
        // memcpy(hmm.initial, pi_new, sizeof(pi_new));
